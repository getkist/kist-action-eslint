# AGENTS.md — kist-action-eslint

`@getkist/action-eslint` is a **kist action plugin**: an npm package whose
default export is an `ActionPlugin` descriptor that kist core loads to register
actions usable from a `kist.yml` pipeline. ESLint linting actions for kist.
...
| --- | --- |
| `LintAction` | Runs ESLint over files/directories, optionally auto-fixing. |

Runtime dependencies: `eslint`.
...
- `eslint` is a runtime dependency here, so the version this plugin lints with
  is the one it pins — not the host project's.
...
```bash
npm run build
# then, from a project with a kist.yml that uses LintAction:
